...
class AuthScreenViewModel @Inject constructor(
            ...
            if (regStatus is DomainResult.Success && regStatus.data.uuid != null) {
                ...
                _state.update {
                    it.copy(
                        ...
                        registrationStatus = regStatus.data.status
                    )
                }
            }
        }
    ...
    fun handleAction(action: AuthScreenAction) {
        when (action) {
            ...
            is AuthScreenAction.OnPrivacyPolicyChecked -> {
                _state.update { it.copy(isPrivacyPolicyAccepted = action.checked) }
            }
            AuthScreenAction.OnPrivacyPolicyClicked -> {
                viewModelScope.launch {
                    _events.emit(AuthScreenEvent.NavigateToPrivacyPolicy)
                }
            }
        }
    }
